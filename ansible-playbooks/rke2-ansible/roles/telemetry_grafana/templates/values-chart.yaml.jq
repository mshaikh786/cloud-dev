...
alertmanager:
  ...
  ## Deploy alertmanager
  ##
  enabled: {{ alertmanager_enabled | default(true) }}
  ...
  ingress:
    enabled: {{ alertmanager_ingress_enabled | default(true) }}
    ...
    ##
    hosts:
      - {{ alertmanager_ingress_host | default('alertmanager.kaust.local') }}
  ...
  ingressPerReplica:
    ...
    ##
    ## Prefix for the per replica ingress that will have   
    ## appended to the end
    hostPrefix: ""
...
grafana:
  ...
  ##
  defaultDashboardsEnabled: {{ grafana_default_dashboards_enabled | default(false) }}
  ...
  ##
  defaultDashboardsTimezone: {{ grafana_timezone | default('Asia/Karachi') }}

  ## Editable flag for the default dashboards
  ##
  defaultDashboardsEditable: {{ grafana_dashboards_editable | default(true) }}

  ## Default interval for Grafana dashboards
  ##
  defaultDashboardsInterval: {{ grafana_dashboards_interval | default('1m') }}
  ...
  ingress:
    ...
    ##
    enabled: {{ grafana_ingress_enabled | default(true) }}
    ...
    ##
    hosts:
      - {{ grafana_ingress_host | default('grafana.kaust.local') }}
  ...
  # #
  persistence:
    enabled: {{ grafana_persistence_enabled | default(true) }}
    type: sts
    storageClassName: "{{ grafana_storage_class | default('nfs-client') }}"
    accessModes:
      - ReadWriteOnce
    size: {{ grafana_storage_size | default('20Gi') }}
    finalizers:
  ...
  additionalDataSources:
    ...
    #   type: loki
    #   access: proxy
    #   url: http://loki-gateway.loki-logs.svc.cluster.local:80
    #   isDefault: false
    #   editable: true
    #   jsonData:
    #     timeout: 60
...
prometheus-node-exporter:
  ...
  releaseLabel: true
  extraArgs:
    - {% raw %}--collector.filesystem.mount-points-exclude=^/(dev|proc|sys|var/lib/docker/.+|var/lib/kubelet/.+)($|/){% endraw %}

    - {% raw %}--collector.filesystem.fs-types-exclude=^(autofs|binfmt_misc|bpf|cgroup2?|configfs|debugfs|devpts|devtmpfs|fusectl|hugetlbfs|iso9660|mqueue|nsfs|overlay|proc|procfs|pstore|rpc_pipefs|securityfs|selinuxfs|squashfs|sysfs|tracefs)${% endraw %}
...
prometheus:
  ...
  # Enable this can make Thanos Query can use
    thanosService:
    enabled: {{ thanos_enabled | default(true) }}
    annotations: {}
    labels: {}
  ...
  thanosIngress:
    enabled: {{ thanos_ingress_enabled | default(true) }}
    ingressClassName: {{ ingress_class_name | default('nginx') }}
    ...
    ##
    hosts:
     - {{ thanos_ingress_host | default('thanos-gateway.kaust.local') }}
  ...
  ingress:
    enabled: {{ prometheus_ingress_enabled | default(true) }}
    ingressClassName: {{ ingress_class_name | default('nginx') }}
    ...
    ##
    hosts:
     - {{ prometheus_ingress_host | default('prometheus.kaust.local') }}
    # hosts: []
  ...
  prometheusSpec:
    ...
    ##
    scrapeInterval: {{ prometheus_scrape_interval | default('') }}

    ## Number of seconds to wait for target to respond before erroring
    ##
    scrapeTimeout: {{ prometheus_scrape_timeout | default('') }}
    ...
    ##
    evaluationInterval: {{ prometheus_evaluation_interval | default('') }}
    ...
    ##
    retentionSize: "{{ prometheus_retention_size | default('25GB') }}"
    ...
    ##
    shards: {{ prometheus_shards | default(1) }}

    ## Log level for Prometheus be configured in
    ##
    logLevel: {{ prometheus_log_level | default('info') }}

    ## Log format for Prometheus be configured in
    ##
    logFormat: {{ prometheus_log_format | default('logfmt') }}
    ...
    resources:
      requests:
        memory: "{{ prometheus_memory_request | default('4Gi') }}"
        cpu: "{{ prometheus_cpu_request | default('2000m') }}"
      limits:
    ...
    ##
      volumeClaimTemplate:
        spec:
          storageClassName: {{ prometheus_storage_class | default('nfs-client') }}
          accessModes: ["{{ prometheus_access_mode | default('ReadWriteMany') }}"]
          resources:
            requests:
    ...
    thanos:
      ...
      objectStorageConfig:
        existingSecret:
          name: {{ thanos_object_storage_secret | default('thanos-object-storage') }}
          key: thanos.yaml
      # secretProviderClass:
      #   provider: gcp
      #   parameters:
